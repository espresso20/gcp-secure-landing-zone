# NIST 800-53 Rev. 5 Moderate: control mapping
...
What it does implement is the technically enforceable subset, meaning the controls whose
satisfaction really is a matter of how the cloud environment is configured, and it leaves an
...
|---|---|---|
| Implemented | This Terraform configures something that materially contributes | 28 controls across 7 families |
| Inherited | Google satisfies it under their own FedRAMP authorization; you consume it | Most of PE, MA, MP, and parts of SC |
| Not applicable, or procedural | Requires a human, a policy document, or an organization | The rest, including all of AT, PS, PL, CP |
...
|---|---|---|
| AC-2 | Account management | `compute.requireOsLogin`, so SSH identity is an IAM identity and account lifecycle is IAM lifecycle. `modules/org-policy-baseline` |
...
| AC-20 | Use of external systems | `iam.allowedPolicyMemberDomains`, so IAM bindings cannot name identities outside your Cloud Identity tenant. Requires `TF_VAR_customer_id`, and is skipped silently if that is unset |
...
| AU-6 | Review and analysis | The Cloud Logging bucket is queryable. The review itself is procedural and not implemented |
...
| AU-12 | Audit record generation | `google_project_iam_audit_config` enables DATA_READ and DATA_WRITE on all services in every project. Those are off by default in GCP and are the ones that tell you who read the data. Plus VPC flow logs and Cloud NAT logging |
...
- PE (Physical and Environmental Protection), entirely. You have no physical access to Google's
  facilities and no ability to control it.
- MA (Maintenance), for the underlying hardware.
- MP (Media Protection), for sanitization and disposal of physical disks.
- SC-5 (Denial of service protection), at Google's edge.
...
A control mapping with no gaps section is not describing a system, it is advertising one. The
gaps below are the part worth reading.

### Procedural, and not expressible in Terraform

Entire families: AT (Awareness and Training), PS (Personnel Security), PL (Planning), CP
(Contingency Planning), PM (Program Management), SR (Supply Chain Risk Management). Plus AU-6's
actual review activity, CA-2 (assessments), CA-7 (continuous monitoring as a programme), and
IR-2 through IR-5, the response capability itself.
...
| AU-9(3) | Locked GCS retention policy on the audit archive | `lock_retention` exists and is off. A locked bucket cannot be deleted until every object ages out, so turning it on strands `make lab-down` for a year. Right for a real deployment, wrong for a playground |

### Deliberately deferred: blast radius

| Control | What would satisfy it | Why not here |
|---|---|---|
| CM-6, AC-3 | `google_org_policy_custom_constraint`, for cases the built-in constraints miss | Custom constraints can only be defined at the organization node, so folder-scoped mode cannot have them and `scripts/guard.sh` blocks the resource type. `stacks/1-org` implements them, but only in an organization explicitly unlocked via `ORG_WRITES_ALLOWED_FOR`. In the default mode this is the main capability the folder rule costs you |
...
**Data access log cost scales with use.** At lab volume it is negligible. Enabling `DATA_READ`
on `allServices` in a busy real-world project is not a decision to copy from here without
measuring it first.

**The audit archive lives in the seed project**, inside the same folder it audits. A
sufficiently privileged compromise of the folder could tamper with both. Proper separation puts
the logging project under a different administrative boundary. That is out of scope at this
budget, but it is a genuine finding and an auditor would raise it.

**No separation of duties.** One person holds every role, so AC-5 is not implemented and cannot
be in a single-operator lab.

**`gcp.resourceLocations` is set to `in:us-locations`.** That is a data-residency choice rather
than a security control. It is here mostly because it also prevents accidental multi-region
...
The mapping is more useful as a study artifact than as a compliance artifact. Given a control,
name the GCP mechanism. Given a GCP mechanism, name what it actually prevents. Practising in
both directions is what the exam rewards.

The exam rarely asks "is this compliant". It asks "which of these four options enforces X across
every project in a folder without letting a project owner override it". The answer is a
