...
A folder-scoped GCP landing zone in Terraform, built to study for the ACE to PCA path and to
have somewhere safe to break things.

It implements the technically enforceable subset of the NIST 800-53 Rev. 5 Moderate baseline and
the GCP well-architected and Enterprise Foundation Blueprint patterns, with one deliberate
deviation from stock EFB that keeps the rest of the organization out of the blast radius.
...
Stock EFB attaches org policies and log sinks at the organization node, where every folder in
the org inherits them, including folders you did not intend to touch. This repo attaches
everything at the folder instead.

`scripts/guard.sh` runs between every `plan` and `apply` and refuses any plan that writes above
the playground folder or mentions a protected identifier. There is no flag to skip it, and
`make guard-test` runs it against 23 fixtures.

Full reasoning: [docs/architecture.md](docs/architecture.md).

For the org-level build instead, meaning real EFB with custom constraints and org-wide sinks,
...
Point the repo at a different org and it re-locks itself, because the setting names an
organization rather than toggling a mode. [docs/org-setup.md](docs/org-setup.md) is the
checklist for standing up an org it is safe in.
...
| `make guard-test` | check the blast-radius guard still works (23 fixtures) |
...
scripts/
  ...
  guard-test.sh               23 fixtures checking the guard still works
modules/
  org-policy-baseline/        folder-scoped org policy, NIST-annotated
  audit-logging/              folder sink to GCS + Cloud Logging, asset feed
...
docs/
  ...
  exam-notes.md               ACE to PCA notes tied to the resources in this repo
```
...
`make preflight` checks all of it and names the grant that is missing.
...
pennies, and is permanent.

**Project IDs are burned on delete.** Hence the random suffix in `modules/project`.

**`make nuke` refuses the state bucket and the folder.** Both carry `prevent_destroy`, which is
intentional.

**This is not a compliance product.** Read [docs/control-mapping.md](docs/control-mapping.md),
particularly the "Not implemented, and why" section, before repeating any claim about NIST in a
room with an auditor in it.
